    ...
    const enrichedClients = useMemo(() => {
        const platformClients = clients.map(c => {
            ...
            return {
                ...
                energy_today: (c as any).energy_today ?? 0,
                last_sync: (c as any).last_sync || null,
                sync_status: (c as any).sync_status || 'IDLE',
                sync_error: (c as any).sync_error || null,
                latestBill: bill,
        ...
        const appSystemsOnly = systems.map(s => {
            return {
                ...
                sync_status: (s as any).sync_status || 'IDLE',
                sync_error: (s as any).sync_error || null,
                last_sync: (s as any).last_sync || null,
                tem_meter: (s as any).tem_meter || false,
                energy_today: (s as any).energy_today || 0,
                generation: (s as any).last_generation || 0,
                latestBill: null,
                status: 'Incompleto',
